...
hotline::Hotline::Hotline(int const ac, char const **av)
{
    ...
    if (ac == 4) {
        if (std::atoi(av[3]) == 0 || std::atoi(av[3]) > 24)
            throw Error(std::cerr, "The number of worked hours must be between 1 and 24, retry with -h.");
        _d = std::atoi(av[1]);
        _c = std::atoi(av[2]);
        _h = std::atoi(av[3]);
    }
    if (ac == 3 && std::atoi(av[1]) < std::atoi(av[2]))
...
static long double factorial(int nbr)
{
    long double res = nbr;
...
static double getBinomialCoef(double n, double k)
{
    if (k == 0)
        return 1;
